...
1. **ベーシックモード**: 10本のピン。ボールを投げてピンを倒せる
2. **得点モード**: 10本のピン、倒れたピンの数を得点として表示する
...
1. オブジェクトを監視して、初期位置から動いたオブジェクトの数をカウントできます。
  1. 監視対象のオブジェクトの形状を `cube` `sphere` `cylinder` から選べます。
  2. カウントする動作を、 `moved`（移動）または `fallen`（転倒）のどちらかを選べます。
...
ゲームアプリはメインスレッドで実行されます。ゲームアプリが終了したら、finallyブロックで指定した「app.game_logic.stop()」が実行され、サブスレッドを安全に停止されています。
