...
```bash
cubicpy --external -gravity 0
```

このコマンドを実行すると、CubicPyはWebsocketサーバーとして起動し、画面左上にルーム番号が表示されます。このルーム番号を使って、Scratch3から接続することができます。複雑な建築をするときは崩壊を防ぐために、「-g 0」を指定して、重力を0にしておきます。

> 🔍 **発見ポイント**: `-x`オプションは「external」（外部）の略で、CubicPyが外部アプリケーションからの接続を受け付けるモードになります。このモードでは、画面左上に表示されるルーム番号が重要です！

### Scratch3側の設定
次に、Scratch3側の設定を行います。今回は、特別な拡張機能を持つScratch3 MODの「Xcratch」を使用します。Xcratchは、Scratch3に新しい機能を追加できるプラットフォームです。CubicPyとの連携には、Scratch3の「CubicPy拡張機能」を使用します。
...
![CubicPy Extension](https://creativival.github.io/CubicPyCode/assets/cubicpy_extension.png)

**▲図1▲ Xcratchに追加したCubicPy拡張**
...
2. **キューブを作るブロック**: 指定した座標にキューブ（立方体）を配置する
3. **キューブのサイズ設定ブロック**: 作成するキューブのサイズを設定する
4. **キューブの設置間隔設定ブロック**: キューブを設置する際の時間間隔を設定する（この機能はCubicPyでは動作しません）
5. **データ送信ブロック**: 設定したキューブのデータをCubicPyに送信する
6. **データクリアブロック**: キューブのデータをクリアする

これらのブロックと **( )回繰り返すブロック** を組み合わせることで、Scratch3からCubicPyの3D世界を自由に操作できます。

> 💡 **先生からのヒント**: CubicPyのブロックはたくさんの種類があるけど、基本的なブロックで作品作りは可能ですよ！　まずは、基本的なブロックの使い方に慣れよう。
...
2. 「CubicPy Turtle」拡張機能が使えるサンプルプロジェクトが開きます。
3. 「CubicPy Turtle」拡張機能のブロックが使えるようになります
...
このプログラムは、オブジェクトの衝突する箇所を条件式で判定して、交差点のオブジェクトを置かないようにしています。
...
2. **データ送信のタイミング**: 多くのキューブを一度に配置する場合は、送信間隔を調整します
3. **処理速度の問題**: 処理が重い場合は、キューブのサイズを大きくして数を減らしてみます
